...
--Number of pages in the buffer cache & the memory consumed by them
SELECT
...
--Same as above but more granularity
SELECT
...
--Percentage of each table that is in memory
...
SELECT
	...
	CAST((CAST(CTE_BUFFER_CACHE.buffer_cache_pages AS DECIMAL) / CAST(PARTITION_STATS.total_number_of_used_pages AS DECIMAL) * 100)
...
--Free space per table in a given database:
SELECT
	objects.name AS object_name,
	objects.type_desc AS object_type_description,
	COUNT(*) AS buffer_cache_pages,
...
--Count the clean vs. dirty pages in the buffer cache
SELECT
    databases.name AS database_name,
...
--Same as above but with more granularity
SELECT
	indexes.name AS index_name,
	objects.name AS object_name,
	objects.type_desc AS object_type_description,
...
--ONLY RUN THIS WHEN YOU WANT TO CLEAR THE BUFFER CACHE
